prompt --application/deployment/install/install_eba_cust_flex_fw_package_body
begin
wwv_flow_api.create_install_script(
 p_id=>wwv_flow_api.id(14880040731996298928)
,p_install_id=>wwv_flow_api.id(17817343456864234187)
,p_name=>'eba_cust_flex_fw package body'
,p_sequence=>510
,p_script_type=>'INSTALL'
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'create or replace package body eba_cust_flex_fw as',
...
'                                                from eba_cust_flex_static_lovs sl',
...
'        delete from eba_cust_flex_page_map;',
'',
'        for c1 in',
'        (',
'            select',
...
'                l_region_type := ''FORM'';',
'            end if;',
'            if instr(c1.static_id, ''_'') > 0 then',
'                -- It''s ''CUSTOMERS_PRODUCTS'' so populate 2 flex_column_prefixes',
'                l_prefix1 := substr(c1.static_id, 1, instr(c1.static_id,''_'') - 2);',
'                l_prefix2 := substr(c1.static_id, instr(c1.static_id,''_'') + 1, length(c1.static_id) - instr(c1.static_id,''_'') - 1);',
'                --insert multiple rows',
'                -- insert row #1',
'                insert into eba_cust_flex_page_map',
'                (id, flex_table, page_id, region_static_id, flex_column_prefix, region_type)',
'                values',
'                (null, ''EBA_CUST_'' || l_prefix1 || ''S'', c1.page_id, c1.static_id, l_prefix1, l_region_type);',
'                -- insert row #2',
'                insert into eba_cust_flex_page_map',
'                (id, flex_table, page_id, region_static_id, flex_column_prefix, region_type)',
'                values',
'                (null, ''EBA_CUST_'' || l_prefix2 || ''S'', c1.page_id, c1.static_id, l_prefix2, l_region_type);',
'            else',
'                -- populate flex_column_prefix',
'                l_prefix1 := substr(c1.static_id, 1, length(c1.static_id) - 1);',
'                --insert one row',
'                insert into eba_cust_flex_page_map',
'                (id, flex_table, page_id, region_static_id, flex_column_prefix, region_type)',
'                values',
...
'            condition1         = ''select null from eba_cust_flex_registry ''',
...
wwv_flow_api.create_install_object(
 p_id=>wwv_flow_api.id(15109397794091858954)
,p_script_id=>wwv_flow_api.id(14880040731996298928)
